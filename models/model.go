...
//select address,area.name area_name,ctime,house.id house_id,index_image_url image_url,order_count,price,room_count,title,user.avatar_url user_avatar   from user,area,house   where user.id = house.user_id     and house.area_id=area.id and user.id=3;
type UserHouseInfo struct {
...
func (this *OrderHouse) To_order_info() interface{} {
	order_info := map[string]interface{}{
		...
		"img_url":    this.House.Index_image_url,
